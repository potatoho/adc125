...
#define MAX_FRAME_BYTES        (MAX_FRAME_WORDS * 8U)
...
/* ============================================================
 * adc_ctrl AXI4-Lite register map
 * ============================================================ */

#ifndef XPAR_ADC_CTRL_0_S00_AXI_BASEADDR
#error "XPAR_ADC_CTRL_0_S00_AXI_BASEADDR is not defined. Regenerate the BSP from the XSA that contains adc_ctrl."
#endif
...
static volatile uint32_t PendingFrameSizeWords = MAX_FRAME_WORDS;
...
{
    return Xil_In32(
        ADC_CTRL_BASEADDR + adc_ctrl_offset(reg)
    );
}


static void adc_ctrl_write(
    uint16_t reg,
    uint32_t value
)
{
...
{
    return sanitize_frame_words(
        adc_ctrl_read(ADC_CTRL_REG_FRAME_SIZE)
    );
}
...
{
    PendingFrameSizeWords =
        sanitize_frame_words(frame_words);
...
static void apply_pending_frame_size(void)
{
    if (!PendingFrameSizeValid)
    {
        return;
...
static uint32_t frame_words_to_bytes(uint32_t frame_words)
{
    return frame_words * 8U;
}
...
static void initialize_adc_control_ip(void)
{
    ...
    adc_ctrl_write(
        ADC_CTRL_REG_PRETRIGGER_SIZE,
        DEFAULT_PRETRIGGER_WORDS
    );

    adc_ctrl_write(
        ADC_CTRL_REG_SAMPLE_DECIMATION,
        DEFAULT_SAMPLE_DECIMATION
    );

    adc_ctrl_write(
        ADC_CTRL_REG_TRIGGER_CFG,
        DEFAULT_TRIGGER_CFG
    );

    adc_ctrl_write(
        ADC_CTRL_REG_SELF_THRESHOLD,
        DEFAULT_SELF_THRESHOLD
    );

    adc_ctrl_write(
        ADC_CTRL_REG_CHANNEL_MASK,
        DEFAULT_CHANNEL_MASK
    );

    xil_printf(
        "adc_ctrl initialized at 0x%08lx\r\n",
...
)
{
    ...
    reply.value =
        htonl(value);
    reply.control =
        htonl(adc_ctrl_read(ADC_CTRL_REG_CONTROL));
    reply.frame_size =
        htonl(get_reported_frame_words());
    reply.pretrigger_size =
        htonl(adc_ctrl_read(ADC_CTRL_REG_PRETRIGGER_SIZE));
    reply.sample_decimation =
        htonl(adc_ctrl_read(ADC_CTRL_REG_SAMPLE_DECIMATION));
    reply.trigger_cfg =
        htonl(adc_ctrl_read(ADC_CTRL_REG_TRIGGER_CFG));
    reply.self_threshold =
        htonl(adc_ctrl_read(ADC_CTRL_REG_SELF_THRESHOLD));
    reply.channel_mask =
        htonl(adc_ctrl_read(ADC_CTRL_REG_CHANNEL_MASK));
...
)
{
    ...
    if (op == CTRL_OP_WRITE)
    {
        ...
        else
        {
            ...
            if (reg == ADC_CTRL_REG_FRAME_SIZE)
            {
                value = get_reported_frame_words();
            }
            else
            {
                value = adc_ctrl_read(reg);
            }
        }
    }
...
static int initialize_udp(void)
{
    ...
    xil_printf(
        ...
        (unsigned long)frame_bytes_to_udp_packets(MAX_FRAME_BYTES)
    );
...
static int initialize_control_udp(void)
{
    ...
    {
        xil_printf(
            "ERROR: control udp_bind failed: %d\r\n",
            err
        );
    ...
    xil_printf(
        "Control UDP listening on port %lu\r\n",
...
)
{
    ...
    const uint16_t total_packets =
        frame_bytes_to_udp_packets(frame_bytes);
...
int main(void)
{
    ...
    {
        xil_printf(
            "Control UDP initialization failed\r\n"
        );
    ...
    active_rx_buffer = 0;
    active_frame_words =
        get_configured_frame_words();
    active_frame_bytes =
        frame_words_to_bytes(active_frame_words);
    ...
    while (1)
    {
        ...
        {
            xil_printf(
                "WARNING: frame %lu UDP transmission failed; arming next DMA\r\n",
